...
describe("POST /peer/transactions", function () {
    it("Using valid transaction with wrong magic in headers. Should fail", function (done) {
        var transaction = node.feynman.transaction.createTransaction("1", 1, message, node.Gaccount.password);
    ...
    it("Using transaction with undefined recipientId. Should fail", function (done) {
        var transaction = node.feynman.transaction.createTransaction(undefined, 1, message, node.Gaccount.password);
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
    ...
    it("Using invalid passphrase. Should fail", function (done) {
        var transaction = node.feynman.transaction.createTransaction("12", 1, message, node.Gaccount.password);
        transaction.recipientId = "1";
        transaction.id = node.feynman.crypto.getId(transaction);
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
    ...
    it("When sender has no funds. Should fail", function (done) {
        var transaction = node.feynman.transaction.createTransaction("1", 1, message, "randomstring");
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
    ...
    it("Using very large amount and genesis block id. Should fail", function (done) {
        var transaction = node.feynman.transaction.createTransaction("12", 10000000000000000, message, node.Gaccount.password);
    ...
    it("Using overflown amount. Should fail", function (done) {
        var transaction = node.feynman.transaction.createTransaction("12", 184819291270000000012910218291201281920128129, message, node.Gaccount.password);
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
    ...
    it("Using float amount. Should fail", function (done) {
        var transaction = node.feynman.transaction.createTransaction("12", 1.3, message, node.Gaccount.password);
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
