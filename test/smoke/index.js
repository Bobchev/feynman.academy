...
async function sendMoneyToAccounts() {
  ...
  for (const i of config.bitcoinValidators) {
    addresses.push(i.feynmanAccount.address)
  }
...
async function testGateway() {
  ...
  for (const validator of config.bitcoinValidators) {
    trs = {
      secret: validator.feynmanAccount.secret,
      type: 2,
      fee: 10000000000,
      args: [validator.name],
    }
    console.log('set name for gateway validator:', validator.feynmanAccount.address)
    await node.transactionUnsignedAsync(trs)
  }
  await node.onNewBlockAsync()
  ...
  for (const validator of config.bitcoinValidators) {
    ...
    console.log('register gateway validator:', validator.feynmanAccount.address)
    await node.transactionUnsignedAsync(trs)
  }
  await node.onNewBlockAsync()
...
async function testGroup() {
  ...
  trs = node.feynman.transaction.createMultiSigTransaction({
    type: 503,
    fee: 100000000,
    senderId: groupAccount.address,
    args: [
  ...
  trs.signatures = []
  for (let i = 0; i < 3; i++) {
    trs.signatures.push(node.feynman.transaction.signMultiSigTransaction(trs, group.members[i].secret))
  }
