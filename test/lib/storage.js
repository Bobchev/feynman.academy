...
function createStorage(content, secret) {
  return node.feynman.storage.createStorage(content, secret)
}
...
describe('Test storage', () => {
  ...
  it('Normal storage test', async function () {
    ...
    var res = await node.submitTransactionAsync(node.feynman.storage.createStorage('1234abcd', account1.password))
