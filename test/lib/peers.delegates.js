...
describe("POST /peer/transactions", function () {
    ...
    describe("Registering a delegate", function () {
                    ...
                    node.api.put("/transactions")
                        ...
                        .end(function (err, res) {
                            node.onNewBlock(function (err) {
                                ...
                                var transaction = node.feynman.delegate.createDelegate(crypto.randomBytes(64).toString("hex"), account.password);
        ...
        it("When account has no funds. Should fail", function (done) {
            var transaction = node.feynman.delegate.createDelegate(node.randomDelegateName().toLowerCase(), node.randomPassword());
            transaction.fee = node.Fees.delegateRegistrationFee;
        ...
        it("When account has funds. Username is uppercase, Lowercase username already registered. Should fail", function (done) {
            var transaction = node.feynman.delegate.createDelegate(account.username.toUpperCase(), account2.password);
        ...
        it("When account has funds. Username is lowercase. Should be ok", function (done) {
            ...
            var transaction = node.feynman.delegate.createDelegate(account.username, account.password);
                    ...
                    node.api.put("/transactions")
                        ...
                        .end(function (err, res) {
                            ...
                            node.onNewBlock(function (err) {
                                ...
                                var transaction = node.feynman.delegate.createDelegate(account2.username, account2.password);
                                ...
                                node.peer.post("/transactions")
                                    ...
                                    .end(function (err, res) {
                                        ...
                                        account2.username = node.randomDelegateName().toLowerCase();
                                        var transaction2 = node.feynman.delegate.createDelegate(account2.username, account2.password);
