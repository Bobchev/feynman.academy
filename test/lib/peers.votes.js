...
describe("POST /peer/transactions", function () {
    ...
    before(function (done) {
        node.api.get("/delegates/")
            ...
            .end(function (err, res) {
                ...
                var transaction = node.feynman.vote.createVote(votes, voterAccount.password);
    ...
    it("Voting twice for a delegate. Should fail", function (done) {
        node.onNewBlock(function (err) {
            var transaction = node.feynman.vote.createVote(["+"+delegate1], voterAccount.password);
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
    ...
    it("Removing votes from a delegate and then voting again. Should fail", function (done) {
        node.onNewBlock(function (err) {
            var transaction = node.feynman.vote.createVote(["-"+delegate2], voterAccount.password);
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
                ...
                .end(function (err, res) {
                    ...
                    node.expect(res.body).to.have.property("success").to.be.true;
                    var transaction2 = node.feynman.vote.createVote(["+"+delegate2], voterAccount.password);
                    node.peer.post("/transactions")
                        .set("Accept", "application/json")
                        .set("version", node.version)
                        .set("magic", node.config.magic)
    ...
    it("Voting for a delegate. Should be ok", function (done) {
        var transaction = node.feynman.vote.createVote(["+" + account.publicKey], account.password);
        node.onNewBlock(function (err) {
            node.expect(err).to.be.not.ok;
            node.peer.post("/transactions")
                .set("Accept", "application/json")
