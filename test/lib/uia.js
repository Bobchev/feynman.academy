...
async function registerIssuerAsync(name, desc, account) {
  var res = await node.submitTransactionAsync(node.feynman.uia.createIssuer(name, desc, account.password))
  DEBUG('register issuer response', res.body)
  return res
}

async function registerAssetAsync(name, desc, maximum, precision, strategy, account) {
  var res = await node.submitTransactionAsync(node.feynman.uia.createAsset(name, desc, maximum, precision, strategy, 1, 1, 1, account.password))
  DEBUG('register asset response', res.body)
  return res
}

async function issueAssetAsync(currency, amount, account) {
  var res = await node.submitTransactionAsync(node.feynman.uia.createIssue(currency, amount, account.password))
  DEBUG('issue asset response', res.body)
  return res
}

async function writeoffAssetAsync(currency, account) {
  var res = await node.submitTransactionAsync(node.feynman.uia.createFlags(currency, 2, 1, account.password))
  DEBUG('writeoff asset response', res.body)
  return res
}

async function changeFlagsAsync(currency, flagType, flag, account) {
  var res = await node.submitTransactionAsync(node.feynman.uia.createFlags(currency, flagType, flag, account.password))
...
async function updateAclAsync(currency, operator, flag, list, account) {
  var res = await node.submitTransactionAsync(node.feynman.uia.createAcl(currency, operator, flag, list, account.password))
  DEBUG('update acl response', res.body)
  return res
}

async function transferAsync(currency, amount, recipientId, account) {
  var res = await node.submitTransactionAsync(node.feynman.uia.createTransfer(currency, amount, recipientId, '', account.password))
...
describe('Test UIA', () => {
  ...
  describe('Normal caces', () => {
    ...
    it('Register issuer should be ok', async function () {
      var trs = node.feynman.uia.createIssuer(ISSUER1.name, ISSUER1.desc, node.Gaccount.password)
    ...
    it('Register asset should be ok', async function () {
      ...
      var trs = node.feynman.uia.createAsset(
        currency,
        ASSET1.desc,
        ASSET1.maximum,
        ASSET1.precision,
    ...
    it('Issue and transfer asset should be ok', async function () {
      ...
      var trs = node.feynman.uia.createIssue(currency, amount, node.Gaccount.password)
      ...
      trs = node.feynman.uia.createTransfer(currency, transferAmount, transferAddress, '', node.Gaccount.password)
    ...
    it('Update flags and acl should be ok', async function () {
      ...
      var trs = node.feynman.uia.createFlags(currency, 1, 1, node.Gaccount.password)
      var [err, res] = await node.submitTransactionAsyncE(trs)
      ...
      trs = node.feynman.uia.createAcl(currency, '+', 1, whiteList, node.Gaccount.password)
      var [err, res] = await node.submitTransactionAsyncE(trs)
      ...
      trs = node.feynman.uia.createTransfer(currency, '10', account1.address, '', node.Gaccount.password)
      res = await node.submitTransactionAsync(trs)
      DEBUG('transfer to account1 response', res.body)
      expect(res.body).to.have.property('success').to.be.true

      trs = node.feynman.uia.createTransfer(currency, '10', account2.address, '', node.Gaccount.password)
      res = await node.submitTransactionAsync(trs)
      DEBUG('transfer to account2 response', res.body)
      expect(res.body).to.have.property('success').to.be.true

      trs = node.feynman.uia.createTransfer(currency, '10', node.genNormalAccount().address, '', node.Gaccount.password)
      res = await node.submitTransactionAsync(trs)
  ...
  describe('Test modify permission', () => {
    ...
    async function registerAssetWithAllowParameters(allowWriteoff, allowWhitelist, allowBlacklist) {
      var trs = node.feynman.uia.createAsset(ASSET_NAME, 'valid desc', MAX_AMOUNT, 1, '', allowWriteoff, allowWhitelist, allowBlacklist, ISSUE_ACCOUNT.password)
