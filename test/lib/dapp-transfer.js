...
let DAPP_TEMPLATE = {
  "name": "feynman-hello-dapp",
  "link": "https://github.com/sqfasd/feynman-hello-dapp/archive/master.zip",
  "category": 1,
  "description": "A hello world demo for feynman dapp",
  "tags": "feynman,dapp,demo",
...
async function registerDAppAsync(options, account) {
  let res = await node.submitTransactionAsync(node.feynman.dapp.createDApp(options, account.password))
  DEBUG('register dapp response', res.body)
  return res
}

async function inTransferAsync(options, account) {
  let res = await node.submitTransactionAsync(node.feynman.transfer.createInTransfer(options.dappId, options.currency, options.amount, account.password))
  DEBUG('in transfer response', res.body)
  return res
}

async function outTransferAsync(options, account) {
  let trs = node.feynman.transfer.createOutTransfer(options.recipientId, options.dappId, options.transactionId, options.currency, options.amount, account.password)
...
describe('dapp transfer', function () {
  ...
  it('should be ok to transfer assets to an app', async () => {
    ...
    let res = await node.submitTransactionAsync(node.feynman.uia.createIssuer(issuerName, 'issuer desc', account.password))
    expect(res.body).to.have.property('success').to.be.true
    await node.onNewBlockAsync()

    let trs = node.feynman.uia.createAsset(
      currency,
      'asset desc',
      maximum,
      6,
    ...
    res = await node.submitTransactionAsync(node.feynman.uia.createIssue(currency, issueAmount, account.password))
    expect(res.body).to.have.property('success').to.be.true
    await node.onNewBlockAsync()
  ...
  it('should be failed to transfer from app to account with invalid params', async () => {
    ...
    let trs = node.feynman.transfer.createOutTransfer(recipientAccount.address, dappId, node.randomTid(), 'XAS', '1', delegateAccounts[0].password)
    res = await node.submitTransactionAsync(trs)
    ...
    for (let i = 0; i < delegateAccounts.length; ++i) {
      trs.signatures.push(node.feynman.transfer.signOutTransfer(trs, delegateAccounts[i].password))
    }
    trs.signatures.push(node.feynman.transfer.signOutTransfer(trs, recipientAccount.password))
    res = await node.submitTransactionAsync(trs)
    ...
    for (let i = 0; i < dapp.unlockDelegates; ++i) {
      trs.signatures.push(node.feynman.transfer.signOutTransfer(trs, node.genNormalAccount().password))
    }
    res =await node.submitTransactionAsync(trs)
    DEBUG('submit out transfer response', res.body)
  ...
  it('should be ok to transfer XAS from app to account', async () => {
    ...
    let trs = node.feynman.transfer.createOutTransfer(recipientAccount.address, dappId, node.randomTid(), 'XAS', amount, delegateAccounts[0].password)
    trs.signatures = []
    for (let i = 0; i < dapp.unlockDelegates; ++i) {
      trs.signatures.push(node.feynman.transfer.signOutTransfer(trs, delegateAccounts[i].password))
    }
  ...
  it('should be ok to transfer assets from app to account', async () => {
    ...
    let trs = node.feynman.transfer.createOutTransfer(recipientAccount.address, dappId, node.randomTid(), currency, amount, delegateAccounts[0].password)
    trs.signatures = []
    for (let i = 0; i < dapp.unlockDelegates; ++i) {
      trs.signatures.push(node.feynman.transfer.signOutTransfer(trs, delegateAccounts[i].password))
    }
